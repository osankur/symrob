...
and array of alpha bounds indexed by clocks (maximal constant appearing in ta for each clock), and a target recognizing function, check for robust reachability *)
  let reach ta  =
    ...
    in
    (*
    let print_lua_bounds l =
      let (lb,ub,ab) = get_lua_bounds l in
      for x = 0 to dim - 1 do
      printf "%s(%s,%s,%s)\n" (VarContext.var_of_index (Model.clocks ta) x ) (Dbm.Alg.to_string lb.(x)) (Dbm.Alg.to_string ub.(x)) (Dbm.Alg.to_string ab.(x));
      done;
      printf "\n";
    in
    *)
    ...
    let expand node =
      ...
      List.iter (fun edge ->             
								 (*
             printf "\n\n\tProcessing edge:\n";
             Model.print_transition stdout ta edge;
						 printf "With guard:\n";
						 let (_,g,_,_) = Model.transition_fields ta edge in
						 Dbm.pretty_print stdout (Model.clocks ta) g;
						 printf "\n\n";
									*)
             (try
    ...
    let expand_gamma_cycle first last =
      (*
      let rec print_the_thing node =
        if ( !Log.log_level = Log.Debug ) then
          (
            Model.print_extended_state stdout ta (BatDllist.get node).label;
            let next = BatDllist.next node in
            if ( not (next == (BatDllist.next last) )) then
              (print_the_thing next)
          )
      in
...
and process them. *)
    ...
    (*
      let print_factor first last =
        if ( !Log.log_level = Log.Debug ) then
          begin
            let rec print_the_thing node =
              Model.print_extended_state stdout ta 
                                         (BatDllist.get node).label;
              let next = BatDllist.next node in
              if ( not (next == (BatDllist.next last) )) then
                (print_the_thing next)
            in
            print_the_thing first;
          end
      in
     *)
      let accelerate_all_factors first last =
        ...
        (*
        Log.debug "Processing factor:\n";
        print_factor first last;
        Log.debug "\n";
         *)
        let len = ref 1 in
      ...
      let same_exact_state = match !Options.gammaGreedyLevel with
        ...
        | _ -> fun s t -> 
							 (*
							 printf "----------\nComparing s and t:\n";
							 Model.print_discrete_state stdout ta (fst s);
							 printf "and\n";
							 Model.print_discrete_state stdout ta (fst t);
...
(*
module QuantEnlargedUBfs = Reachability(ZoneQueue)(EnlargedUppaalAutomaton)
module QuantEnlargedUDfs = Reachability(ZoneStack)(EnlargedUppaalAutomaton)
 *)
module Dfs = Reachability(ZoneStack)(EnlargedUautomaton)
module Bfs = Reachability(ZoneQueue)(EnlargedUautomaton)
(*module VBfs = Reachability(ZoneQueue)(EnlargedUppaalAutomaton)*)
