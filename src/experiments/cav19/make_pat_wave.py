...
class TAWRITER:
    ...
    def node_output_true(self, node):
        if node[1] == "and":
            ...
            gn = "!(out{0} == 1 &amp;&amp; out{1} == 1)".format(inps[0][0], inps[1][0])
            return (g, gn)
        elif node[1] == "or":
            inps = node[2]
            g = "!(out{0} == 0 &amp;&amp; out{1} == 0)".format(inps[0][0], inps[1][0])
    ...
    def dump_cyclic_graph(self, bound = None):
        ...
        print >> decl, "var err : {0..1} = 0;"
        for inode,node in enumerate(graph):
            i = node[0]
            print >> decl, "var out{0} : {{0..2}} = 2;".format(i)
            ins = []
            if node[1] == "and" or node[1] == "or":
                print >> decl, "var b{0}_0 : {{0..2}} = 2;".format(i)
                print >> decl, "var b{0}_1 : {{0..2}} = 2;".format(i)
            elif node[1] == "th":
                for x in range(len(node[3])):
                    print >> decl, "var b{0}_{1} : {{0..2}} = 2;".format(i,x)

            if node[1] != "in":
                proc = Template("Node{0}".format(i))
                if i == 0:
                    proc.decl = proc.decl + "clock: t, x0;\n".format(i)
                else:
                    proc.decl = proc.decl + "clock: x{0};\n".format(i)
                ...
                if bound == None or inode != 0:
                    proc.add_transition(Transition(down, up, guard="{1} &lt;= x{0} &amp;&amp; x{0} &lt;= {2} ".format(i, self.delays[i][0], self.delays[i][1]), disc_guard =  node_ready[0]  + " &amp;&amp; " + node_outputs_true[0], disc_up="out{0} = 1".format(i)))

                else:
                    bg = "t &lt;= {0}".format(bound)
                    proc.add_transition(Transition(down, up, guard="{1} &lt;= x{0} &amp;&amp; x{0} &lt;= {2} &amp;&amp; {3}".format(i, self.delays[i][0], self.delays[i][1], bg), disc_guard = node_ready[0]  + " &amp;&amp; " + node_outputs_true[0], disc_up="out{0} = 1".format(i)))
                    non_input_out_reset = ";".join(map(lambda n: "out{0} = 2".format(n[0]), filter(lambda n: n[1] != "in", graph)))
                    # this is news
                    proc.add_transition(Transition(down,down, guard="t &gt;= {0}".format(bound+1), disc_up=non_input_out_reset))

                proc.add_transition(Transition(down, up, guard="{1} &lt;= x{0} &amp;&amp; x{0} &lt;= {2} ".format(i, self.delays[i][0], self.delays[i][1]), disc_guard = node_ready[0]  + " &amp;&amp; " + node_outputs_true[1], disc_up="out{0} = 0".format(i)))
...
#                proc.add_transition(Transition(down, up, guard="x{0} &lt;= {1} ".format(i, self.delays[i][0]) + " &amp;&amp; " + node_ready[0]  + " &amp;&amp; " + node_outputs_true[1], up="out{0} := 0".format(i)))
                proc.add_transition(Transition(down, down, guard="x{0} == {1} ".format(i, self.delays[i][0]), disc_guard =  node_ready[1], reset="x{0}".format(i)))

                proc.add_transition(Transition(up, down, guard="x{0} &lt;={1}".format(i, self.delays[i][1]), reset="x{0}".format(i), disc_up = "out{0} = 2".format(i)))

                if inode == 0:
                    proc.add_transition(Transition(up,Location("err",error=True), disc_guard="out{0}==1".format(i), disc_up="err = 1;"))
        ...
        for node in input_nodes:
            ...
            proc.add_transition(Transition(prev_loc, loc, disc_up="out{0} = 1".format(node[0])))
            proc.add_transition(Transition(prev_loc, loc, disc_up="out{0} = 0".format(node[0])))
            prev_loc = loc
        proc.add_transition(Transition(prev_loc, Location("end")))

        nta.add_template(proc)
        print >> decl, "System = " + "||| ".join(map(lambda x: x.name + "()" , nta.templates)) + ";"
        print >> decl, "#define Err err==1;"
        print >> decl, "#assert System reaches Err;"
...
def main():
    ...
    for i,gr in enumerate(graph):
        ...
        with open("{0}a{1}.ta".format(out_dir,i),"w") as f:
            sys.stdout = f
        # tawriter.test();
    ...
    for i,gr in enumerate(graph_sat):
        if gr == None:
            continue
        tawriter = TAWRITER(gr, args.time, args.numeric, 3000)
        with open("{0}b{1}.ta".format(out_dir, i),"w") as f:
            sys.stdout = f
        # tawriter.test();
        # print >> sys.stderr, "Given #clocked_latches: ", args.clocked_latches
        # tawriter.dump_model(args.clocked_latches, args.bound)
