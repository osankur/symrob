...
#        self.delays = [[5,10,15], [4,8,16]]
    ...
    def make_template(self, nmachines, decl):
...
#            mg = "mode{0} == {1}".format(id, m)
        ...
        for xi, x in enumerate(latch_list):
            ...
            disc_up="{0} = {1};".format(self.name_of(x.lit), next_funcs[x.lit])
                    ...
                    for waits in powerset(range(nprocs)): # for each set of waiting processes
                        ...
                        for mode in product(*ranges):
                            ...
                            for wi, w  in enumerate(waits):
                                ...
                                wgcs = "&amp;&amp;".join(wgc)
                                temp.add_transition(Transition(idle, busy, guard = "&amp;&amp;".join(filter(lambda x: x <> "", [wg, nwg, wgcs])), sync="go{0}!".format(sel), disc_up="running{0} = {1};".format(id, sel)))
...
"""
        ...
        for m in mach_temps:
            nta.add_template(m)
        print >> decl, "System = Process0() ||| Process1() ||| Process2() ||| Machine0() ||| Machine1();"
        print >> decl, "#define Dead dead==1;"
